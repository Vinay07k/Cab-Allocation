...
void prog()
{
    ...
    string A;
    char a;
    insertTrie(rootTrie,"bar");
    insertTrie(rootTrie,"saloon");
    insertTrie(rootTrie,"home");
    insertTrie(rootTrie,"park");
    insertTrie(rootTrie,"mall");
    insertTrie(rootTrie,"theater");
    insertTrie(rootTrie,"casino");
    insertTrie(rootTrie,"restaurant");
    ...
    while(a=='Y'|| a=='y')
        {
    cout<<"enter the initial letters of the desination you want to enter: ";
    cin>>A;
    int comp = printAutoSuggestions(rootTrie,A);

    if (comp == 0)
    {
        cout << "Sorry NO place is There with the given Substring. Wanna Search Again ? (Y/N)";
        cin>>a; }
        cout<<"Wanna Search Again?(Y/N)";
        cin>>a;
   }
